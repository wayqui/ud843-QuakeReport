...
public class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray jsonFeatures = jsonEarthquake.getJSONArray("features");

            for (int i = 0; i < jsonFeatures.length(); i++) {
                JSONObject jsonProperties = jsonFeatures.getJSONObject(i).getJSONObject("properties");

                String strMagnitud = jsonProperties.getString("mag");
                String strUbicacion = jsonProperties.getString("place");
                String strFechaHora = jsonProperties.getString("time");
                String strRutaWebsite = jsonProperties.getString("url");

                Float magnitud = Float.valueOf(strMagnitud);
                Calendar calendar = Calendar.getInstance();
                Date fecha = new Date(Long.valueOf(strFechaHora));
                //SimpleDateFormat dateFormatter = new SimpleDateFormat("MMM DD, yyyy");
                //String dateToDisplay = dateFormatter.format(fecha);

                earthquakes.add(new Earthquake(magnitud, strUbicacion, fecha, strRutaWebsite));
            }
